const VERSION = 'v7-fast-navigate';
...
// Минимальный аварийный HTML, показывается ТОЛЬКО если сама оболочка
// приложения отсутствует во всех кэшах — не должно случаться в норме,
// но гарантирует, что мы НИКОГДА не провалимся в нативный офлайн-экран браузера.
...
// Гонка между сетью и таймаутом: если сеть не отвечает быстро, сразу
// переключаемся на кэш, не дожидаясь долгого системного таймаута,
// который может позволить браузеру показать свой офлайн-экран раньше нас.
function timeout(ms) {
...
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    try {
      ...
      // Активируем новую версию сразу только после УСПЕШНОГО кэширования,
      // чтобы не подменять рабочий SW в момент отсутствия сети.
      self.skipWaiting();
    } catch (e) {
      // Если офлайн во время проверки обновления — install может упасть.
      // Не вызываем skipWaiting — старый активный SW продолжает работать.
    }
  })());
});
...
self.addEventListener('fetch', event => {
  ...
  // НАВИГАЦИОННЫЕ ЗАПРОСЫ — покрывает и холодный старт с домашнего экрана,
  // и ручное обновление/pull-to-refresh открытого PWA. Гонка сеть-vs-таймаут
  // (1200 мс) гарантирует, что при плохой/отсутствующей сети мы отдаём кэш
  // очень быстро, не дожидаясь системного таймаута, который раньше позволял
  // браузеру показать нативный "You're offline" экран поверх приложения.
  if (req.mode === 'navigate') {
    event.respondWith((async () => {
      try {
        const fresh = await Promise.race([fetch(req), timeout(1200)]);
        if (fresh && fresh.ok) {
          const shell = await caches.open(APP_SHELL);
  ...
  // ВСЕ ОСТАЛЬНЫЕ ЗАПРОСЫ ТОГО ЖЕ ORIGIN — cache-first с фоновой
  // ревалидацией и безопасным фолбэком, который никогда не отклоняется.
  event.respondWith((async () => {
    try {
